...
#ligoPos1 = np.argmax(ligoDat > -0.0265, axis=0)[0]
ligoPos1 = np.argmax(ligoDat > -0.012, axis=0)[0]
...
#plt.hlines(0, -0.0265, 0.006, color='grey')
plt.hlines(0, -0.012, 0.006, color='grey')
...
H1fgw_times = np.zeros(4)
L1fgw_times = np.zeros(4)
for i in range(4):
...
plt.figtext(0.26,0.72,r'$\frac{df^{-8/3}}{dt}$', fontsize=14)
plt.figtext(0.33,0.72,r'$=-3.4\times 10^{-4}$', fontsize=10)
plt.legend()
plt.xlabel("Time (s)")
...
plt.figtext(0.26,0.72,r'$\frac{df^{-8/3}}{dt}$', fontsize=14)
plt.figtext(0.33,0.72,r'$=-4.5\times 10^{-4}$', fontsize=10)
plt.legend()
plt.xlabel("Time (s)")
plt.ylabel(r'(Frequency/Hz)$^{-8/3}$')
plt.show()
...
print("H1 frequency-time slope:", round(H1_f_slope,7))
print("L1 frequency-time slope:", round(L1_f_slope,7))
...
print("Estimated masses:", np.round(m[0], 2), "and", np.round(m[1], 2), "solar masses")
